...
const ChatContainer = () => {
  ...
  return selectedUser ?  (
    <div className='h-full flex flex-col relative bg-[#121212] overflow-hidden select-none'>
      {/* HEADER */}
      <div className='flex items-center justify-between h-16 px-6 border-b border-white/5 bg-[#1c1c1e] flex-shrink-0'>
        <div className='flex items-center gap-3'>
          <div className='relative'>
            ...
            {!selectedUser.isGroup && onlineUser.includes(selectedUser._id) && (
              <span className='absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-green-500 border-2 border-[#1c1c1e] animate-pulse'></span>
            )}
          </div>
          <div className='flex flex-col leading-tight'>
            <p className='text-sm font-semibold text-gray-100'>{selectedUser.isGroup ? selectedUser.groupName : selectedUser.fullname}</p>
        ...
        <div className='flex items-center gap-3'>
          <button onClick={()=>setSelectedUser(null)} className='md:hidden p-2 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-all'>
            <img src={assets.arrow_icon} className='max-w-5 opacity-80' />
          </button>
          {selectedUser.isGroup && (
            <button onClick={() => setIsGroupInfoOpen(true)} className='p-2 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-all' title='Group Details'>
            ...
            <React.Fragment key={msg._id || index}>
              {showDivider && (
                <div className='flex justify-center my-6 w-full'>
                  <span className='bg-white/5 border border-white/5 text-neutral-400 text-[10px] font-bold uppercase tracking-widest px-3.5 py-1 rounded-lg shadow-sm'>
              ...
              <div 
                id={`msg-${msg._id}`} 
                className={`flex items-start gap-3.5 my-4 group relative w-full ${isSentByMe ? 'justify-end' : 'justify-start'} ${highlightMessageId === msg._id ? 'bg-[#00f0ff]/5 p-2 rounded-2xl ring-1 ring-[#00f0ff]/30 shadow-xl transition-all duration-300' : ''}`}
              >
                {/* Hover Action Menu */}
                <div className={`absolute top-0 -translate-y-[85%] hidden group-hover:flex items-center gap-1.5 p-1.5 bg-[#1c1c1e] border border-white/10 rounded-xl shadow-2xl z-20 ${isSentByMe ? 'right-4' : 'left-4'}`}>
                  ...
                  <button onClick={()=>setReplyingTo(msg)} className='p-1 rounded text-neutral-400 hover:text-white hover:bg-white/10 text-[10px] transition-colors cursor-pointer' title='Reply'>Reply ↩️</button>
                  {isSentByMe && (
                    <>
                      <button onClick={()=>handleStartEdit(msg)} className='p-1 rounded text-neutral-400 hover:text-[#00f0ff] hover:bg-white/10 text-[10px] transition-colors cursor-pointer' title='Edit'>Edit ✏️</button>
                      <button onClick={()=>handleDelete(msg._id)} className='p-1 rounded text-neutral-400 hover:text-red-400 hover:bg-white/10 text-[10px] transition-colors cursor-pointer' title='Delete'>Delete 🗑️</button>
                    </>
                  )}
                </div>
                ...
                <div className={`flex flex-col ${isSentByMe ? 'items-end' : 'items-start'} max-w-[65%]`}>
                  ...
                  {selectedUser.isGroup && !isSentByMe && (
                    <span className='text-[10px] text-[#00f0ff] font-bold mb-1 ml-1'>{getSenderName(msg.senderId)}</span>
                  )}

                  {/* Deleted message bubble */}
                  {msg.deleted ? (
                    <p className={`p-3 text-xs italic bg-white/3 border border-white/5 text-neutral-500 rounded-xl ${isSentByMe ? 'rounded-tr-none' : 'rounded-tl-none'}`}>
                    ...
                    <div className='flex flex-col gap-1.5 min-w-[200px]'>
                      <input 
                        ...
                        autoFocus 
                        className='p-2.5 w-full text-sm rounded-xl bg-white/5 text-white border border-white/10 outline-none focus:border-[#00f0ff]/50'
                      />
                      <div className='flex gap-2 text-[10px] px-1 font-semibold'>
                        ...
                        <button onClick={handleCancelEdit} className='text-neutral-400 hover:text-neutral-350 cursor-pointer'>Cancel</button>
                      </div>
                    </div>
                  ) : msg.image ? (
                    <div className='relative'>
                      {msg.replyTo && !msg.replyTo.deleted && (
                        <div className='bg-white/5 border-l-2 border-[#00f0ff] rounded-lg px-2.5 py-1.5 mb-1.5 text-xs text-gray-300 max-w-[230px] backdrop-blur-md'>
                          <p className='text-[#00f0ff] text-[10px] font-semibold mb-0.5'>{getSenderName(msg.replyTo.senderId)}</p>
                    ...
                    <div className='relative group/bubble'>
                      {msg.replyTo && !msg.replyTo.deleted && (
                        <div className='bg-white/3 border-l-2 border-[#00f0ff] rounded-lg px-2.5 py-1.5 mb-1.5 text-xs text-neutral-400 max-w-[280px] md:max-w-[320px]'>
                          <p className='text-[#00f0ff] text-[10px] font-bold mb-0.5'>{getSenderName(msg.replyTo.senderId)}</p>
                          {msg.replyTo.image ? <span>🖼️ Photo</span> : <span className='line-clamp-1 text-[11px]'>{msg.replyTo.text}</span>}
                        </div>
                      )}
                      <p className={`p-3 px-4 text-[13px] leading-relaxed rounded-2xl shadow-sm break-words border transition-all ${isSentByMe ? 'bg-[#1c1c1e] border-white/5 text-white rounded-tr-none' : 'bg-white/3 border-white/5 text-gray-200 rounded-tl-none'}`}>
                        {msg.text}
                  ...
                  <span className='text-[9px] text-neutral-500 font-semibold mt-1 px-1'>{formatMessageTime(msg.createdAt)}</span>
                </div>
        ...
        {typingInfo && (
          <div className='flex items-start gap-3 px-4 py-1 flex-row'>
            ...
            <div className='flex flex-col items-start'>
              ...
              <span className='text-[9px] text-neutral-500 font-semibold mt-1 px-1'>{typingInfo.name} typing</span>
            </div>
          </div>
        )}
      ...
      <div className='p-4 bg-[#1c1c1e]/60 border-t border-white/5 backdrop-blur-md flex-shrink-0'>
        {/* Reply Preview */}
        {replyingTo && (
          <div className='flex items-center justify-between bg-[#00f0ff]/5 border-l-2 border-[#00f0ff] rounded-xl px-4 py-2 mb-3 text-sm text-gray-300 backdrop-blur-md max-w-5xl mx-auto'>
            <div className='flex-1 min-w-0'>
              <p className='text-[#00f0ff] text-xs font-bold'>Replying to {getSenderName(replyingTo.senderId)}</p>
              <p className='truncate text-xs mt-0.5 text-neutral-300'>{replyingTo.image ? '🖼️ Photo' : replyingTo.text}</p>
            </div>
            <button onClick={()=>setReplyingTo(null)} className='text-neutral-400 hover:text-white ml-2 text-md cursor-pointer p-1'>✕</button>
          </div>
        )}
        ...
        <div className='flex items-center gap-3.5 max-w-5xl mx-auto'>
          <div className='flex-1 flex items-center bg-white/3 border border-white/5 px-4.5 rounded-xl transition-all focus-within:border-[#00f0ff]/30 focus-within:ring-1 focus-within:ring-[#00f0ff]/30'>
            ...
            <input 
              ...
              className='flex-1 text-sm py-3 bg-transparent border-none outline-none text-white placeholder-neutral-500'
            />
      ...
      {isGroupInfoOpen && selectedUser && selectedUser.isGroup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md transition-all duration-300">
          <div className="bg-[#1c1c1e] border border-white/10 w-full max-w-md p-6 rounded-2xl shadow-2xl flex flex-col gap-4 text-white mx-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col items-center gap-3 text-center">
                <div className="relative">
                  ...
                  />
                  {isAdmin && (
                    <label htmlFor="edit-group-avatar" className="absolute bottom-0 right-0 bg-[#00f0ff] text-black rounded-full p-1.5 cursor-pointer text-xs shadow-md transition-colors">
                      📸
                      <input 
                        type="file" 
                ...
                {isAdmin ? (
                  <div className="flex items-center gap-2 w-full max-w-xs justify-center">
                    <input 
                      ...
                      className="bg-white/5 border border-white/10 rounded-lg p-1.5 px-2.5 text-sm text-center font-bold outline-none focus:ring-1 focus:ring-[#00f0ff] w-full"
                    />
                  </div>
                ) : (
                ...
                <div className="flex justify-between items-center text-xs text-neutral-400 font-semibold uppercase tracking-wider mb-1">
                  ...
                  {isAdmin && (
                    <button 
                      ...
                      className="text-[#00f0ff] hover:text-cyan-300 font-bold transition-colors cursor-pointer text-[11px]"
                    >
                      ➕ Add Members
                    </button>
                  )}
                    ...
                    return (
                      <div key={p._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                          {isMemberAdmin && <span className="text-[10px] font-bold text-[#00f0ff] bg-[#00f0ff]/10 border border-[#00f0ff]/20 rounded px-1.5 py-0.5">Admin</span>}
            ...
            <div className="flex justify-between gap-3 mt-4 pt-3 border-t border-white/5">
              ...
              <button 
                ...
                className="px-6 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-sm font-semibold transition-colors"
              >
                Close
              </button>
            </div>
      ...
      {isAddMembersOpen && selectedUser && selectedUser.isGroup && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/75 backdrop-blur-md transition-all duration-300">
          <div className="bg-[#1c1c1e] border border-white/10 w-full max-w-sm p-6 rounded-2xl shadow-2xl flex flex-col gap-4 text-white mx-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
                ...
                users.filter(u => !selectedUser.participants.some(p => p._id === u._id)).map((user) => (
                  <label key={user._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 cursor-pointer transition-colors">
                    ...
                    <input 
                      ...
                      className="w-4 h-4 rounded border-gray-300 text-[#00f0ff] focus:ring-[#00f0ff] bg-white/5 border-white/10"
                    />
                  </label>
                ))
              )}
    ...
    <div className='flex-1 flex flex-col items-center justify-center gap-4 text-neutral-500 bg-[#121212] max-md:hidden'>
      <div className='p-8 rounded-2xl bg-[#1c1c1e] border border-white/5 shadow-2xl flex flex-col items-center gap-4 text-center max-w-sm animate-fade-in'>
        <img src={assets.logo_icon} className='max-w-12 filter drop-shadow-[0_2px_12px_rgba(0,240,255,0.25)] animate-pulse' alt="" />
        <div>
          <p className='text-md font-bold text-gray-200 tracking-wide'>QuickChat Enterprise</p>
          <p className='text-xs text-neutral-400 leading-relaxed mt-2'>Select a contact or group conversation from the sidebar to start messaging. All conversations are secured with client-side end-to-end encryption.</p>
        </div>
      </div>
    </div>
  )
