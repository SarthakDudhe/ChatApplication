...
const Sidebar = () => {
  ...
  const highlightText = (text, highlight) => {
    ...
    return (
      <span>
        {parts.map((part, i) => 
          regex.test(part) ? (
            <mark key={i} className="bg-[#00f0ff]/30 text-white rounded px-0.5 font-semibold">{part}</mark>
          ) : (
            part
          )
        )}
  ...
  return (
    <div className={`bg-[#1c1c1e] border-r border-white/5 h-full p-4 text-white flex flex-col select-none transition-all duration-300 ${selectedUser?"max-md:hidden":""}`}>
        ...
        <div className='pb-4 flex-shrink-0 border-b border-white/5'>
            <div className='flex justify-between items-center'>
                <div className='flex items-center gap-2.5 cursor-pointer' onClick={() => navigate('/profile')}>
                    <div className='relative'>
                      ...
                      <span className='absolute bottom-0 right-0 w-2 h-2 rounded-full bg-green-500 border border-[#1c1c1e]'></span>
                    </div>
                    <div className='flex flex-col text-left leading-none'>
                      <span className='text-sm font-semibold text-gray-100 hover:text-[#00f0ff] transition-colors'>{authUser?.fullname || 'Profile'}</span>
                      <span className='text-[10px] text-neutral-400 mt-0.5'>Active now</span>
                    </div>
                </div>
                ...
                <div className='flex items-center gap-1.5'>
                    ...
                    <button 
                      ...
                      className='p-2 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-all duration-200' 
                    ...
                    <div className='relative py-1 group'>
                        <button className='p-2 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-all duration-200'>
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                        ...
                        <div className='absolute top-full right-0 z-20 w-52 p-4 rounded-xl bg-[#1c1c1e] border border-white/5 text-gray-100 hidden group-hover:block shadow-2xl backdrop-blur-xl animate-fade-in'>
                          <p onClick={()=>navigate('/profile')} className='cursor-pointer text-sm hover:text-[#00f0ff] transition-colors font-medium flex items-center gap-2'>
                            👤 Edit Profile
                          </p>
                          ...
                          <div className='flex flex-col gap-3.5 my-2.5 text-xs text-gray-300'>
                            <div className='flex justify-between items-center'>
                              ...
                              <label className="relative inline-flex items-center cursor-pointer">
                                ...
                                />
                                <div className="w-8 h-4 bg-white/10 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-[#00f0ff]"></div>
                              </label>
                            </div>
                            <div className='flex justify-between items-center'>
         ...
         <div className='bg-white/3 border border-white/5 rounded-xl flex items-center gap-3 py-2.5 px-4 mt-4 transition-all focus-within:border-[#00f0ff]/30 focus-within:ring-1 focus-within:ring-[#00f0ff]/30'>
          <img src={assets.search_icon} alt="Search" className='w-3.5 opacity-40'/>
          <input 
            type="text" 
            value={input}
            onChange={(e)=>setInput(e.target.value)} 
            className='bg-transparent border-none outline-none text-white text-xs placeholder-[#606060] flex-1' 
      ...
      <div className='flex-1 overflow-y-auto pr-1 mt-4 flex flex-col gap-5'>
        ...
        {filteredGroups.length > 0 && (
          <div>
            <p className='text-[10px] text-neutral-500 font-bold uppercase tracking-widest px-2 mb-2 flex justify-between items-center'>
              <span>👥 Groups</span>
              <span className='bg-white/5 text-neutral-400 text-[9px] px-1.5 py-0.5 rounded font-mono'>{filteredGroups.length}</span>
            </p>
            <div className='flex flex-col gap-1.5'>
              {filteredGroups.map((group) => {
                ...
                return (
                  ...
                  >
                    {isSelected && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 rounded-r-md bg-[#00f0ff] shadow-[0_0_8px_#00f0ff]"></span>
                    )}
                    ...
                    <div className='flex flex-col leading-tight flex-1 min-w-0'>
                      <p className='font-semibold text-sm text-gray-200 truncate'>{group.groupName}</p>
                      {isTyping ? (
                        <span className='text-[#00f0ff] text-[10px] font-medium mt-0.5 animate-pulse'>{typingText}</span>
                      ) : (
                        <span className='text-neutral-450 text-[10px] mt-0.5 truncate text-neutral-400'>
                          {group.participants.map(p => p.fullname).join(", ")}
                        </span>
                      )}
                    </div>
                    {unseenMessages[group._id] > 0 && (
                      <p className='absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#d300c5] text-white shadow-[0_0_8px_rgba(211,0,197,0.3)] animate-pulse'>
        ...
        <div>
          <p className='text-[10px] text-neutral-500 font-bold uppercase tracking-widest px-2 mb-2 flex justify-between items-center'>
            ...
            <span className='bg-white/5 text-neutral-400 text-[9px] px-1.5 py-0.5 rounded font-mono'>{filteredUsers.length}</span>
          </p>
          <div className='flex flex-col gap-1.5'>
            {filteredUsers.length === 0 ? (
              <p className='text-xs text-neutral-500 px-2 py-1'>No contacts found</p>
            ) : (
                ...
                >
                  ...
                  <div className='flex flex-col leading-tight flex-1 min-w-0'>
                    ...
                    {typingUsers[user._id] ? (
                      <span className='text-[#00f0ff] text-[10px] font-medium mt-0.5 animate-pulse'>typing...</span>
                    ) : onlineUser.includes(user._id) ? (
        ...
        {input.trim() !== "" && (
          <div className='border-t border-white/5 pt-4'>
            <p className='text-[10px] text-neutral-500 font-bold uppercase tracking-widest px-2 mb-2'>Search Results</p>
            {isSearching ? (
              <p className='text-xs text-neutral-550 px-2 py-1 animate-pulse text-neutral-450'>Searching...</p>
              ...
              <div className='flex flex-col gap-2'>
                {searchResults.map((msg) => (
                  <div 
                    ...
                    className='bg-white/2 hover:bg-white/5 p-3 rounded-xl cursor-pointer transition border border-white/5 hover:border-[#00f0ff]/30 flex flex-col gap-1.5'
                  >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-shrink-0">
                  ...
                  />
                  <label htmlFor="group-avatar-input" className="absolute bottom-0 right-0 bg-[#00f0ff] hover:bg-[#00dcf5] text-black rounded-full p-1 cursor-pointer text-[10px] shadow-md transition-colors">
                    📸
                    <input 
                      type="file" 
                ...
                <div className="flex-1">
                  <input 
                    ...
                    className="w-full p-2.5 rounded-xl bg-white/5 border border-white/10 outline-none text-sm text-white focus:ring-1 focus:ring-[#00f0ff] placeholder-neutral-500"
                  />
                </div>
              </div>
